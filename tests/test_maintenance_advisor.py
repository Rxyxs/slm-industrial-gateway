"""Tests del MaintenanceAdvisorAgent (`src/agents/maintenance_advisor.py`):
interpretación de los resultados crudos de `calculate_rul`/`sensor_anomaly_check`
contra umbrales de urgencia fijos, y comportamiento advisor (nunca lanza
excepción, a diferencia de `SafetyComplianceAgent`).
"""
...
from src.agents.maintenance_advisor import (
...
@pytest.fixture()
def agent() -> MaintenanceAdvisorAgent:
...
def test_assess_never_raises_regardless_of_input(agent):
    ...
    MaintenanceAdvisorAgent es puramente advisor -- ninguna entrada debe producir una excepción."""
...
def test_custom_thresholds_are_respected():
    agent = MaintenanceAdvisorAgent(rul_urgent_threshold_hours=100.0, anomaly_urgent_count=1)
