"""PdMAgent: capa de negocio de mantenimiento predictivo, después de que
`AnalyticsAgent` ejecuta `calculate_rul` o `sensor_anomaly_check`.

Esas tools ya calculan el número (horas de RUL, o los Z-scores/índices
anómalos); este agente no repite ese cálculo -- lo interpreta contra un
umbral de urgencia FIJO y decide si la respuesta debe llevar una alerta de
mantenimiento. Mismo patrón que `SafetyComplianceAgent` aplica sobre límites
físicos de diseño (ver `safety_agent.py`), pero sobre salud de equipo en vez
de límites de seguridad -- y por eso, a diferencia de `SafetyComplianceAgent`,
es deliberadamente **advisor, no bloqueante**: un RUL bajo es información
